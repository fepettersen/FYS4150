...
\begin{align}\label{general solution}
 u(x,t) = e^{pt}e^{inx}
\end{align}
if we insert this expression \ref{general solution} into the scheme \ref{FE} we get
\begin{align*}
 e^{inx}e^{pt}(e^{-t}-1) = Ce^{pt}(e^{i(n+1)x}-2e^{inx} +e^{i(n-1)x})\\
 (e^{-t}-1) = C(e^{ix}-2+e^{-ix})\\
 (e^{-t}-1) = 2C(cos(x)-1) = -4C\sin^2(x/2)
\end{align*}
